	...
	if (!empty($address) || !empty($phone) || !empty($email)) {
		?><div class="contacts_info"><?php
			...
			if (!empty($phone) || !empty($email)) {
                ...
                if (!empty($phone)) {
                    $phone_expl = explode(";", $phone);
                    ?><span class="contacts_phone"><?php
                    foreach ($phone_expl as $value) {
                        $phone_href = $value;
                        $phone_href = preg_replace("/[^0-9]/", '', $value);
                        echo '<a href="tel:'.esc_attr($phone_href).'">'.esc_attr($value).'</a>';
                    }
